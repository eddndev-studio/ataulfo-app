...
class _FakeRepo implements PublicCatalogRepository {
  ...
  ({bool enabled, String slug})? lastUpdate;
  ...
  Future<PublicCatalogSettings> update({
    ...
    lastUpdate = (enabled: enabled, slug: slug);
...
void main() {
  const loaded = PublicCatalogSettings(enabled: false, slug: null, url: null);
  ...
  group('save', () {
    test(
      ...
      () async {
        ...
        final cubit = PublicCatalogCubit(repo);
        await cubit.load();
        await cubit.save(enabled: true, slug: 'mi-tienda');
        expect(repo.lastUpdate, (enabled: true, slug: 'mi-tienda'));
    ...
    test(
      ...
      () async {
        ...
        final cubit = PublicCatalogCubit(repo);
        await cubit.load();
        await cubit.save(enabled: true, slug: 'ocupado');
