...
void main() {
  ...
  group('DioTriggersDatasource.createTrigger happy path', () {
    test(
      'POST /templates/:templateId/triggers TEXT con body completo',
      () async {
        when(
  ...
  group('DioTriggersDatasource.createTrigger failure mapping', () {
    ...
    test(
      '422 → TriggersInvalidFailure',
      () => expectMappedTo<TriggersInvalidFailure>(422),
    );
    test(
      '403 → TriggersForbiddenFailure',
      () => expectMappedTo<TriggersForbiddenFailure>(403),
    );
    test(
      '404 → TriggersNotFoundFailure (template padre)',
      () => expectMappedTo<TriggersNotFoundFailure>(404),
    );
    test(
      '503 → TriggersServerFailure',
      () => expectMappedTo<TriggersServerFailure>(503),
    );
  });

  group('DioTriggersDatasource.updateTrigger happy path', () {
    test(
      'PUT /triggers/:id TEXT con documento completo (sin flowId)',
      () async {
        when(
    ...
    test(
      'PUT LABEL preserva campos del modo (sin keyword/matchType)',
      () async {
        when(
          () => dio.put<Map<String, dynamic>>(
            any(),
  ...
  group('DioTriggersDatasource.updateTrigger failure mapping', () {
    ...
    test(
      '404 → TriggersNotFoundFailure (trigger gone)',
      () => expectMappedTo<TriggersNotFoundFailure>(404),
    );
    test(
      '403 → TriggersForbiddenFailure',
